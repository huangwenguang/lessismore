        ...
        <div class="col-xs-12 col-md-12">
            <div class="widget">
                ...
                <div class="widget-body">
                    <form id="saveOrUpdate" class="form-horizontal form-bordered" role="form" enctype="multipart/form-data" method="post" action="${ ctx}/protype/save.do">
                    	<input type="hidden" id = "id" name="id" value="${bean.id }"/>
                    	<c:if test="${empty bean.id}">
                    		<div class="form-group">
	                        	<label for="inputEmail3" class="col-sm-2 control-label no-padding-right">开发类型</label>
	                        	<div class="col-sm-10">
	                        		<input type="text" class="form-control" id="name" name="name" 
	                        				placeholder="开发类型" data-bv-notempty="true" data-bv-notempty-message="开发类型不能为空" />
	                        	</div>
                        	</div>                        	
                        </c:if>
                        <c:if test="${!empty bean.id}">
...
<script>
	...
	var save =function(){
		...
		$("#saveOrUpdate").ajaxSubmit(function(result) {
			  if(result.status == 1){
				  window.location.href = '${ctx}' + "/protype/list.do";
			  }else{
